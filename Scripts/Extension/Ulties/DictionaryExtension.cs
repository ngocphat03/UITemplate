namespace UITemplate.Scripts.Extension.Ulties {
    ...
    public static class DictionaryExtension {
        ...
        /// <returns></returns>
        /// <remarks>uses LINQ (for now) so do not use anywhere performance critical</remarks>
        public static Dictionary<TKey, TValue> Merge<TKey, TValue>(this IReadOnlyDictionary<TKey, TValue> source,
            params IReadOnlyDictionary<TKey, TValue>[] dictionaries) {
            var combinedDict = source == null ? new Dictionary<TKey, TValue>() : source.Copy();
        ...
        public static string TryGetString(this IReadOnlyDictionary<string, object> source, string key)
            => source.TryGetValue(key, out var temp) ? temp.ToString() : null;
