namespace AXitUnityTemplate.Blueprint.BlueprintReader.Converter
{
    using System;
    using System.Linq;
    using UnityEngine;
    ...
    using AXitUnityTemplate.Blueprint.BlueprintReader.Converter.TypeConversion;
    ...
    public static class CsvHelper
    {
        ...
        {
            CsvHelper.TypeConverterCache.AddConverter(type, typeConverter);
        }

        public static ITypeConverter GetTypeConverter(Type type)
        {
            return CsvHelper.TypeConverterCache.GetConverter(type);
        }
        ...
        public static T GetField<T>(this CsvDataReader csvReader, string name)
        {
            ...
            return (T)CsvHelper.GetField(csvReader, typeof(T), index);
        }
        ...
        {
            if (CsvHelper.MemberInfosCache.TryGetValue(typeInfo, out var results)) return results;
